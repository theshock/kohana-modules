...
class Kohana_LessCSS {
	...
	public static function compile($in, $out, $force = false) {
	...
	public static function compileAll ($config = null) {
		if ($config === null) $config = Kohana::config('lesscss');
		foreach ($config->files as $in => $out) {
			self::compile($in, $out, $config->force_update);
		}
	}
}
